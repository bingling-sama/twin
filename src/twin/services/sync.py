...
def get_sync_status() -> dict:
    ...
    if s["running"] and s["started_at"] is not None and s["total_files"] > 0:
        ...
        s["elapsed_ms"] = metrics["elapsed_ms"]
        s["rate_img_per_s"] = metrics["rate_img_per_s"]
        s["eta_ms"] = metrics["eta_ms"]
        s["progress_pct"] = metrics["progress_pct"]
    else:
        s["elapsed_ms"] = 0.0
        s["eta_ms"] = 0.0
        s["progress_pct"] = 0.0
...
def sync_images_dir() -> dict:
    ...
    total_batches = (
        (len(to_index) + settings.batch_size - 1) // settings.batch_size if to_index else 0
    )
    ...
    if not to_index:
        ...
        logger.info(
            "All %d images already indexed (%d skipped) in %.1fs", total_files, skipped, elapsed
        )
        with _sync_lock:
